...
bit F_Enter_Get_Ntc;	//进入S_Get_Ntc标志位
...
uint8 Get_Ntc_Count(void)
{
	...
	if( Get_Adc_Avg() )
	{
		if( CHS ==  AI2_ACM)//当前采集的是AI2通道（NTC2）
		{
			g_AIN2Count = g_AdcSum;//保存AI2通道的采样平均值
			Drv_Adc_Channel_Set(AI1_ACM); //切换到AI1通道（NTC1）
		}
        else    //这里不能时if判断，否则会直接返回1，AI1_ACM通道采集失败
        {
            g_AIN1Count = g_AdcSum;//保存AI1通道的采样平均值
			Drv_Adc_Channel_Set(AI2_ACM);//切换回AI2通道，准备下一轮
			return 1;	//完整的一轮采集
        }
	}
	return 0;
...
{
#if ET_FT != 1
    if ((eTestmode_num == Earmode) && (eReadyTask_Sta == Ready_ReadyOk)  && !uErrFlag.bits.Er2)//耳温模式下，且系统准备就绪，且NTC无错误
    {
        if( Get_Adc_SingleRead() )
        {
            g_AdcCount ++;
            Get_Ear_Tp_Max(g_AdcData);//检测热电堆峰值（用于耳温测量）
        }
    }
    else
#endif
    {
        if( Get_Ntc_Count() )//完成一次双通道NTC采集
        {
            Ntc_Caculate();//NTC数据计算
			//g_NtcCount=g_NtcTableUp1-10;
            NtcTable_Check();//NTC数据范围校验
            if( eTestmode_num == Insptectmode )//检测模式下
            {
                NtcTableWider_Check();//执行更宽范围的NTC校验
            }
            NtcTable_Find();//查表获取温度值
        }
