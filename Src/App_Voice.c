...
#define MISO(X)  (X?(FP11=1):(FP11=0))
#define MOSI(X)  (X?(FP12=1):(FP12=0))
#define SCK(X)   (X?(FP13=1):(FP13=0))
...
函数功能：  语音播放参数设置读取函数
输入参数：  SetGetFlag: 2：清空参数 1:存入参数 0：读取参数  表明是清空，存入，输出。
           StatueIn：语音函数参数
           *pStatueOut:通过指针返回参数
输出参数：  无
返回值  ：  无
...
{
    /*最大设置参数深度*/
    static ePlayVoice StatueParam[4];   
    /*SetStatueParamNo：已存入的数据的个数，GetStatueParamNo：已取出数据的个数*/ 
    ...
    /*Flag=0时，取出存入至数组的值，并通过*pStatueOut返回*/ 
    else
    {
        /*如果数组非0*/ 
        /*注意：枚举类型的第一个成员不应该设置成0，会应发此处的逻辑错误*/ 
...
{
    static ePlayVoice eAMK_PlayStatus;
    ...
    switch( eAMK_PlayStatus )
    {
        ...
        case Play_Temp:
            if( !Port_MISO )
            {
                ...
                eAMK_PlayStatus = Play_Wait;   //测量结果播报完毕必定播报clean语音
...
{
    FP11M = 0;  //miso input mode，high level
    FP12M = 1;  //mosi output mode，low level
    FP13M = 1;  //SCK output mode，low level
    FP16M = 1;  //ce output mode, high level
    P1 |= 0x42;   //01000010
}
